...
$temporaryRoot = Join-Path ([System.IO.Path]::GetTempPath()) "codex-snow-skin-tests-$PID-$([guid]::NewGuid().ToString('N'))"
New-Item -ItemType Directory -Path $temporaryRoot | Out-Null

try {
  $configPath = Join-Path $temporaryRoot 'config.toml'
  $backupPath = Join-Path $temporaryRoot 'config.before-snow-skin.toml'
  ...
  if ($null -eq $fakeInstall -or $fakeInstall.PackageFullName -cne $fakePackage.PackageFullName -or
    $fakeInstall.ApplicationId -cne 'CodexApp' -or
  ...
  if ((Get-DreamSkinCodexAppUserModelId -Codex $fakeInstall) -cne 'OpenAI.Codex_test!CodexApp') {
    throw 'The packaged Codex application identity was not composed safely.'
  }
  ...
  $verifiedPackageState.codexPackageFamilyName = $fakePackage.PackageFamilyName
  $verifiedPackageState.codexApplicationId = 'WrongApp'
  if ($null -ne (Resolve-DreamSkinCodexInstallFromState -State $verifiedPackageState `
    -RegisteredInstalls @($fakeInstall))) {
    throw 'A mismatched Appx application ID was accepted from state.'
  }
  ...
  foreach ($scriptPath in Get-ChildItem -LiteralPath (Join-Path $Root 'scripts') -Filter '*.ps1') {
    ...
    $null = [System.Management.Automation.Language.Parser]::ParseFile(
      $scriptPath.FullName, [ref]$tokens, [ref]$parseErrors)
    if ($parseErrors.Count -gt 0) {
      throw "PowerShell parse failure in $($scriptPath.Name): $($parseErrors[0].Message)"
    }
  }
  ...
  if (@(Get-ChildItem -LiteralPath $temporaryRoot -Recurse -File -Filter '*.replace-backup').Count -gt 0 -or
    @(Get-ChildItem -LiteralPath $temporaryRoot -Recurse -File -Filter '*.tmp').Count -gt 0) {
    throw 'Atomic writes left temporary replacement files behind.'
  }

  $node = Get-DreamSkinNodeRuntime
  & $node.Path --check (Join-Path $Root 'scripts\injector.mjs')
  ...
  $payloadResult = @(& $node.Path (Join-Path $Root 'scripts\injector.mjs') --check-payload 2>&1)
  if ($LASTEXITCODE -ne 0) { throw 'Injector self-test failed.' }
  $payloadJson = ($payloadResult -join "`n") | ConvertFrom-Json
  $theme = (Read-DreamSkinUtf8File -Path (Join-Path $Root 'assets\theme.json')) | ConvertFrom-Json
  if (-not $payloadJson.pass -or "$($payloadJson.version)" -cne "$($theme.version)") {

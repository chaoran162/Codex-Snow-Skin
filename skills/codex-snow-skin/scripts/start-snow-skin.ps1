...
try {
  ...
  if ($savedIsDifferent) {
    ...
    if ($currentProcesses.Count -gt 0 -and ($savedProcesses.Count -gt 0 -or $savedOwnsPort)) {
      throw 'Multiple registered Codex package versions are active. Close them manually before starting Snow Skin.'
    }
    if ($savedProcesses.Count -gt 0 -or $savedOwnsPort) {
      ...
      if ($null -ne $savedIdentity) {
        ...
        Write-Warning 'Reapplying Snow Skin to the still-running registered Codex version; the current Store version will be used after that app exits.'
      } else {
        $codexToStop = $savedCodex
  ...
  if (-not $debugReady -and $codexProcesses.Count -gt 0) {
    ...
    if (-not $restartAuthorized -and $PromptRestart) {
      $restartAuthorized = Confirm-DreamSkinRestart -Message 'Codex must restart once to enable Snow Skin. Unsaved input may be lost. Restart now?'
      if (-not $restartAuthorized) {
        Write-Host 'Snow Skin launch was cancelled; Codex was not changed.'
        exit 0
      }
    }
    if (-not $restartAuthorized) {
      throw 'Codex is open without a verified Snow Skin CDP endpoint. Close it first or explicitly use -RestartExisting.'
    }
  ...
  try {
    if ($null -eq (Get-DreamSkinVerifiedCdpIdentity -Port $Port -Codex $codex)) {
      ...
      Start-DreamSkinCodex -Codex $codex -ArgumentList $arguments | Out-Null
    ...
    $stableIdentity = Get-DreamSkinVerifiedCdpIdentity -Port $Port -Codex $codex
    if ($null -eq $stableIdentity -or $stableIdentity.BrowserId -cne $stableBrowserId) {
      throw 'The Codex debugging endpoint did not remain stable after startup.'
    }
  ...
  try {
    ...
    if (-not $recordedInjectorStopped) {
      ...
      Write-Warning "Archived stale Snow Skin state at $staleStatePath"
    }
  } catch {
    if ($launchedWithCdp) {
      try {
  ...
  try {
    ...
    if ($verifyExitCode -ne 0) { throw "Snow Skin verification failed. See $VerifyPath" }
  } catch {
    $startupError = $_
  ...
  Write-Host "Codex Snow Skin is active on verified loopback port $Port."
} finally {

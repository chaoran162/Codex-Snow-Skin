...
function Enter-DreamSkinOperationLock {
  ...
  $mutex = [System.Threading.Mutex]::new($false, "Local\CodexSnowSkin.$sid.Operation")
  $acquired = $false
  try {
  ...
  if (-not $acquired) {
    $mutex.Dispose()
    throw 'Another Codex Snow Skin install, start, restore, or verify operation is already running.'
  }
  return $mutex
}
...
function Initialize-DreamSkinAppActivationInterop {
  if ('CodexDreamSkin.PackagedAppActivation' -as [type]) { return }
...
function Get-DreamSkinCodexAppUserModelId {
  ...
  $familyName = "$($Codex.PackageFamilyName)"
  $applicationId = "$($Codex.ApplicationId)"
  if ($familyName -notmatch '^[A-Za-z0-9._-]+$' -or $applicationId -notmatch '^[A-Za-z0-9._-]+$') {
    throw 'The Codex Store application identity is missing or invalid.'
  }
...
function Start-DreamSkinCodex {
  ...
  Initialize-DreamSkinAppActivationInterop
  try {
    return [CodexDreamSkin.PackagedAppActivation]::Activate($appUserModelId, $arguments)
  } catch {
    throw "Codex Store package activation failed for $appUserModelId. $($_.Exception.Message)"
  }
}
...
function Get-DreamSkinNodeRuntime {
  ...
  $candidates = @()
  $bundledRoot = Join-Path $env:LOCALAPPDATA 'OpenAI\Codex\runtimes\cua_node'
  if (Test-Path -LiteralPath $bundledRoot) {
    $candidates += @(Get-ChildItem -LiteralPath $bundledRoot -Directory -ErrorAction SilentlyContinue |
  ...
  foreach ($candidate in @($candidates | Select-Object -Unique)) {
    try {
      ...
      $major = 0
      if ($LASTEXITCODE -ne 0 -or -not $version -or -not $runtimePath -or
        -not (Test-Path -LiteralPath $runtimePath) -or
        -not [int]::TryParse(($version -split '\.')[0], [ref]$major) -or $major -lt $MinimumMajor) {
        throw "unsupported or unreadable runtime ($version)"
      }
      return [pscustomobject]@{
        Path = $runtimePath
        Version = $version
        Major = $major
        Source = if (Test-DreamSkinPathWithin -Path $runtimePath -Root $bundledRoot) { 'codex-bundled' } else { 'PATH' }
      }
    } catch {
      $failures += "$candidate : $($_.Exception.Message)"
    }
  }
  $details = if ($failures.Count) { " Checked: $($failures -join '; ')" } else { '' }
  throw "Node.js $MinimumMajor or newer was not found. Open Codex once to provision its bundled runtime, or install Node.js $MinimumMajor+.$details"
}
...
function ConvertTo-DreamSkinCodexInstall {
  ...
  $applicationId = $null
  if ($Package.PSObject.Properties.Name -contains 'ApplicationId') {
    $applicationId = "$($Package.ApplicationId)"
  } else {
    try {
      $applications = @((Get-AppxPackageManifest -Package $Package -ErrorAction Stop).Package.Applications.Application)
      $match = @($applications | Where-Object {
  ...
  if ($applicationId -notmatch '^[A-Za-z0-9._-]+$') { return $null }
  return [pscustomobject]@{
...
function Resolve-DreamSkinCodexInstallFromState {
  ...
  foreach ($install in $RegisteredInstalls) {
    ...
    if ($State.codexApplicationId -and "$($State.codexApplicationId)" -cne $install.ApplicationId) {
      continue
    }
    return [pscustomobject]@{
...
function Read-DreamSkinState {
  ...
  try {
    ...
    throw "Snow Skin state is unreadable; it was preserved for inspection: $Path"
  }
}
...
function Stop-DreamSkinRecordedInjector {
  ...
  if (-not $identityMatches) {
    Write-Warning "Skipped stale injector PID $processId because its visible identity does not match the saved Snow Skin process."
    return $false
  }
  ...
  if (Get-Process -Id $processId -ErrorAction SilentlyContinue) {
    throw "The recorded Snow Skin injector did not stop: PID $processId"
  }
  return $true
}
...
function Confirm-DreamSkinRestart {
  ...
  return $shell.Popup($Message, 0, 'Codex Snow Skin', 52) -eq 6
}

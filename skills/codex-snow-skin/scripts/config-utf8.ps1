...
function Assert-DreamSkinTomlLineEditingSafe {
  ...
  if ($Content.Contains('"""') -or $Content.Contains("'''")) {
    throw 'Refusing to rewrite TOML containing multiline strings; use single-line values before installing Snow Skin.'
  }
  foreach ($match in [regex]::Matches($Content, '(?m)^[^\r\n]*=[\t ]*\[[^\r\n]*\r?$')) {
    if ((Get-DreamSkinTomlArrayBracketBalance -Line $match.Value) -ne 0) {
      throw 'Refusing to rewrite TOML containing multiline arrays; use single-line arrays before installing Snow Skin.'
    }
  }
...
function Install-DreamSkinBaseTheme {
  ...
  try {
    ...
    $settings = [ordered]@{
      ...
      appearanceLightChromeTheme = 'appearanceLightChromeTheme = { accent = "#1F7FE8", contrast = 64, fonts = { code = "Cascadia Code", ui = "Microsoft YaHei UI" }, ink = "#10263F", opaqueWindows = true, semanticColors = { diffAdded = "#A9DFC5", diffRemoved = "#F1AEB5", skill = "#54A8E8" }, surface = "#F7FCFF" }'
    }

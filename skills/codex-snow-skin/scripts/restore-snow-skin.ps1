...
try {
  ...
  if ($shouldCloseCodex -and $PromptRestart) {
    $restartMessage = if ($NoRelaunch) {
      'Restore will close Codex and remove Snow Skin plus its CDP session. Continue?'
    } else {
      'Restore will close Codex, remove Snow Skin and its CDP session, then reopen the official app. Continue?'
    }
  ...
  $backup = Join-Path $StateRoot 'config.before-snow-skin.toml'
  ...
  Write-Host 'Snow Skin restore actions completed; any saved CDP session was closed.'
} finally {
